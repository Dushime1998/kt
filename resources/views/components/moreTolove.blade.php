...
<div class=" mx-auto">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700">
            ...
            @else
            <span class="bg-red-500 text-white px-1 py-0.5 rounded-lg text-xs">{{ $accessory->Badge }}</span>
            @endif
            <a href="{{ route('products_show', ['productId' => $accessory->id]) }}" class="block overflow-hidden">
                <img src="{{ asset('uploads/' . $array[0]) }}" alt="{{ $accessory->productName }}" class="w-full h-24 object-cover rounded-t-lg transition-transform hover:scale-110" />
            </a>
            <div class="px-4 py-4">
                <a href="{{ route('products_show', ['productId' => $accessory->id]) }}">
                    <h5 class="text-xs truncate-text font-semibold tracking-tight text-gray-900 dark:text-white">
                        {{ substr($accessory->productName, 0, 10) }}{{ strlen($accessory->productName)> 15 ? '...' : '' }}
                    </h5>
                </a>
                <div class="flex items-center justify-between mt-2">
                    <h1 class="font-semibold text-xs">
                        <span class="text-xs">Rwf</span>
                        <span class="text-xs font-bold text-gray-900 dark:text-white">{{ $accessory->productPrice }}</span>
                    </h1>
                    <a href="{{ route('addTocart', ['id' => $accessory->id]) }}" class="rounded-lg tooltip text-black" data-tip="Add To Cart">
                        <?xml version="1.0" encoding="utf-8"?>
                        <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                        <svg width="20px" height="20px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-labelledby="cartAddIconTitle" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none" color="#000000">
